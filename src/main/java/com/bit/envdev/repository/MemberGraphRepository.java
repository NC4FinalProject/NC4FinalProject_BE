...
public interface MemberGraphRepository extends JpaRepository<MemberGraph, MemberGraphId> {
    @Query(value = "SELECT DATE(m.created_at) AS registration_date, COUNT(*) AS user_count " +
            ...
            "GROUP BY DATE(m.created_at), YEAR(m.created_at), MONTH(m.created_at)", nativeQuery = true)
    List<MemberGraph> findMemberGraph();

    @Query(value = "SELECT m1_0.created_at as registration_date, COUNT(*) as user_count FROM member m1_0 GROUP BY m1_0.created_at, YEAR(m1_0.created_at), MONTH(m1_0.created_at)", nativeQuery = true)
    List<MemberGraph> findMonthTotalUserCount();

    @Query(value = "SELECT m.created_at as registration_date, COUNT(*) as user_count FROM member m WHERE m.created_at >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR) GROUP BY m.created_at, YEAR(m.created_at), MONTH(m.created_at)", nativeQuery = true)
